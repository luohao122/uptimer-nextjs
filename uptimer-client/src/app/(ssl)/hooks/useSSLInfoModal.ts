...
export const useSSLInfoModal = (
  monitor: ISSLMonitorDocument
): IUseSSLInfoModal => {
  const sslInfo: ISSLInfo = monitor.info
    ? JSON.parse(monitor.info)
    : sslDefaultInfoData;
  const hostInfo: SSLModalDataProp[] = [
    { key: "Name", value: sslInfo.host ?? "-" },
  ];
  const subjectInfo: SSLModalDataProp[] = [
    { key: "Valid for Domains", value: sslInfo.subject.common_name ?? "-" },
    {
      key: "Subject Alternative Name (SAN)",
      value: sslInfo.subject.sans ?? "-",
    },
  ];
