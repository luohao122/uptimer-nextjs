...
export const useUptimeView = (params: { slug: string[] }): IUseUptimeView => {
  ...
  const { data } = useQuery(GET_HEART_BEATS, {
     ...
     */
    fetchPolicy: JSON.parse(`${searchParams.get("active")}`)
      ? "no-cache"
      : "cache-first",
  ...
  const [toggleMonitor] = useMutation(TOGGLE_MONITOR, {
    update(_, { data: { toggleMonitor } }) {
      const monitors = toggleMonitor.monitors.filter(
        (monitor: IMonitorDocument) =>
          parseInt(`${monitor.id}`) === parseInt(`${monitorId}`, 10)
      );
    ...
    if (data) {
      ...
      if (result.length > 0) {
        ...
        const sum = result.reduce(
          (acc: number, heartbeat: IHeartbeat) => acc + heartbeat.responseTime,
          0
        );
  ...
  const toggleUserMonitor = async (): Promise<void> => {
    ...
    router.push(
      `/uptime/view/${monitor.type}/${monitorId}/${duration}?${params}`
    );
    try {
      toggleMonitor({
        variables: {
